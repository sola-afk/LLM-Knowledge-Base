...
created: 2026-05-08
type: requirements
derived_from: Researcher/research-call-supervision-audit.md
---

# Detection Criteria — Call Monitoring Agent v1

What the call monitoring agent must detect, ranked by priority and mapped to the regulation or risk that motivates each detection. Each criterion is grounded in at least one breach actually flagged by the human compliance reviewer in the **☎️ Call Supervision Audit** (June 2025 – March 2026), so the Designer can build prompts against documented behaviour rather than hypothetical risk.
...
### HF-01 — Explicit personal recommendation
...
### HF-02 — Comparative value judgement on regulated products or schemes
...
### HF-04 — Cross-selling or arranging a regulated product without authorisation
...
### HF-05 — Tax, social-welfare, or pensions-law guidance to a specific person/scenario
...
1. **Criterion ID** (HF-01 … SF-15, ES-01 …)
2. **Severity** (High / Medium / Low / Escalate)
3. **Hard or Soft** (so Designer can apply the co-occurrence rule)
4. **Evidence quote** — the exact transcript span that triggered the detection
5. **Timestamp** — start/end as given by the transcript (matches reviewer convention)
6. **Recommended action** — one of: "Coach (language)", "Coach (perimeter)", "Refer to compliance", "Escalate to SME"
