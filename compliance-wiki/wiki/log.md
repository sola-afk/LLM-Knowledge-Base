...
## [2026-08-13] monitoring | Embed Call Monitoring Q2 2026 — full coverage (5 of 5)
- Sources: transcripts for the remaining 3 recordings supplied by the Head of Compliance (locally auto-transcribed, Whisper tiny.en)
- Updated: output/embed-call-monitoring-2026-q2.md — added the 15/06 contractual terms call (F18-F26), the 17/06 technical session (F27-F35), and the 17/06 Justworks touchpoint (F36-F42); added a priority-findings section, transcript provenance and quality caveats, full register rows and actions A1-A26
- Coverage: 5 of 5. 42 findings, 1 NC.
- Most material: (1) Smart Pension asked whether it needed appointed representative status and the question was not answered; (2) the member journey triages insurance queries through the unregulated partner's account managers by Slack, with "advice" and "guidance" used interchangeably; (3) new joiners auto-enrolled into cover with no IPID or pre-contractual information anywhere in the designed journey; (4) no DPIA, DSA or lawful basis raised at any point across the design, including purpose extension to non-members and dependants; (5) payroll deduction relies on a control the counterparty states does not work, with no reconciliation.
- Also new: named-competitor exclusivity discussed with no legal input (flagged for specialist advice, not concluded on); insurer approached on a prospective client's behalf with no mandate; non-EEA distribution through third-party brokers with no due diligence; partnership discussion moving onto WhatsApp.
- Positives recorded: consent to record sought on the technical session (by the counterparty); Trevor Gardiner's product explanation accurate and clear; on the Justworks call the Embed lead declined to guess three times on product questions and used an anonymised case study.
- Notes: Q1 remediation targeted customer-facing sales calls and held — no advice to a consumer on any of the five. The same failure modes reappeared one layer up in the design of the distribution arrangements, where no script applies and no compliance gate exists. Compliance, Legal, the DPO and product governance appear in none of the five conversations.
- Transcript caveat: recordings 2, 4 and 5 are heavily garbled small-model transcriptions. Figures in particular are unreliable — F18 and F25 must be confirmed against audio before use.
- Gaps for lint: no concept pages for [[Minimum Competency Code]], financial promotions, the appointed representative regime, or outsourcing / third-party distribution chains.
